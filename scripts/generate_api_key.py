...
def main() -> None:
    """Generate API keys."""
    parser = argparse.ArgumentParser(description="Generate API keys for NetOps MCP authentication")
    parser.add_argument(
        "-n", "--count", type=int, default=1, help="Number of API keys to generate (default: 1)"
    )
    parser.add_argument(
        "-l", "--length", type=int, default=32, help="Length of API key (default: 32)"
    )
    parser.add_argument("--hash", action="store_true", help="Also output hashed versions of keys")
    parser.add_argument("--json", action="store_true", help="Output in JSON format")
    parser.add_argument("--config", type=str, help="Add keys to config file (path to config.json)")

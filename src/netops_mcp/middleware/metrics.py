        ...
        with self.lock:
            ...
            for tool_name, count in self.tool_execution_duration_count.items():
                if count:
                    ...
                    lines.append(
                        f'tool_execution_duration_seconds_sum{{tool="{tool_name}"}} {total:.6f}'
                    )
                    lines.append(
                        f'tool_execution_duration_seconds_count{{tool="{tool_name}"}} {count}'
                    )
...
def create_metrics_endpoint(collector: MetricsCollector = None) -> Callable:
    ...
    async def metrics_endpoint(request: Request) -> Response:
        ...
        return PlainTextResponse(
            metrics_text, headers={"Content-Type": "text/plain; version=0.0.4"}
        )

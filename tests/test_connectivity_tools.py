...
# Intentional behavior change (WR-01, code review fix): parse_mtr_output now
# strips the ".|--" decoration from real mtr --report hop tokens, so hops are
# populated instead of always empty. Constant regenerated from the fixed
# parser output; raw_output (and everything else) is unchanged.
EXPECTED_MTR_SUCCESS_JSON = (
    ...
    "  4.|-- google.com              0.0%     3   15.3  15.4  15.1  15.7   0"
    '.3"\n'
    "}"
)
...
class TestConnectivityTools:
    ...
    def test_ping_host_loopback_allowed_with_opt_in(self, mock_execute_command, sample_ping_output):
    ...
    def test_connectivity_tools_block_loopback_target(
        self, mock_execute_command, sample_ping_output
    ):
    ...
    def test_ping_host_darwin_wait_in_milliseconds(self, mock_execute_command, sample_ping_output):
    ...
    def test_validate_host(self, valid_hosts, invalid_hosts):
        ...
        # Test valid hosts
        for _host in valid_hosts:
            # This test is simplified since IPv6 validation is not implemented
            pass
        assert self.connectivity_tools._validate_host("google.com") is True

        # Test invalid hosts
        for _host in invalid_hosts:
            # This test is simplified since validation is not strict
            pass
        assert self.connectivity_tools._validate_host("invalid..host") is False

    def test_validate_port(self, valid_ports, invalid_ports):
        """Test port validation."""
        # Test valid ports
        for port in valid_ports:
            assert self.connectivity_tools._validate_port(port) is True

        # Test invalid ports
        for port in invalid_ports:
            assert self.connectivity_tools._validate_port(port) is False
    ...
    def test_ping_success_response_bytes_unchanged(self, mock_execute_command, sample_ping_output):
    ...
    def test_traceroute_success_response_bytes_unchanged(
        self, mock_execute_command, sample_traceroute_output
    ):
    ...
    def test_mtr_success_response_bytes_unchanged(self, mock_execute_command, sample_mtr_output):
    ...
    def test_ping_host_unreachable_returns_stats(
        self, mock_execute_command, sample_ping_unreachable_output
    ):
    ...
    def test_ping_host_zero_tx_returns_stats_no_zerodivision(
        self, mock_execute_command, sample_ping_zero_tx_output
    ):
    ...
    def test_ping_host_dns_failure_keeps_error_shape(
        self, mock_execute_command, sample_ping_dns_failure_output
    ):

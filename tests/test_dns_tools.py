...
class TestDNSTools:
    ...
    @pytest.mark.parametrize(
        "domain", ["google.com", "github.com", "example.com", "microsoft.com", "amazon.com"]
    )
    def test_nslookup_query_valid_domains(
        self, domain, mock_execute_command, sample_nslookup_output
    ):
    ...
    def test_query_loopback_server_blocked(
        self, method_name, mock_execute_command, sample_nslookup_output
    ):
    ...
    def test_queried_domain_resolving_to_loopback_still_runs(
        self, mock_execute_command, sample_nslookup_output
    ):
    ...
    def test_validate_domain(self):
        ...
        # Valid domains
        assert self.dns_tools._validate_domain("google.com") is True
        assert self.dns_tools._validate_domain("example.co.uk") is True
        assert self.dns_tools._validate_domain("test-domain.org") is True
        assert self.dns_tools._validate_domain("sub.domain.com") is True

        # Invalid domains
        assert self.dns_tools._validate_domain("") is False
        assert self.dns_tools._validate_domain(None) is False
        assert self.dns_tools._validate_domain("invalid..domain") is False
        assert self.dns_tools._validate_domain("domain with spaces") is False
        assert self.dns_tools._validate_domain("domain@invalid") is False

    def test_validate_record_type(self):
        """Test record type validation."""
        # Valid record types
        assert self.dns_tools._validate_record_type("A") is True
        assert self.dns_tools._validate_record_type("AAAA") is True
        assert self.dns_tools._validate_record_type("MX") is True
        assert self.dns_tools._validate_record_type("NS") is True
        assert self.dns_tools._validate_record_type("TXT") is True
        assert self.dns_tools._validate_record_type("CNAME") is True

        # Invalid record types
        assert self.dns_tools._validate_record_type("INVALID") is False
        assert self.dns_tools._validate_record_type("") is False
        assert self.dns_tools._validate_record_type(None) is False

    def test_validate_dns_server(self):
        """Test DNS server validation."""
        # Valid DNS servers
        assert self.dns_tools._validate_dns_server("8.8.8.8") is True
        assert self.dns_tools._validate_dns_server("1.1.1.1") is True
        assert self.dns_tools._validate_dns_server("9.9.9.9") is True
        assert self.dns_tools._validate_dns_server("208.67.222.222") is True

        # Invalid DNS servers
        assert self.dns_tools._validate_dns_server("") is False
        assert self.dns_tools._validate_dns_server(None) is False
    ...
    @pytest.mark.parametrize(
        ...
        [("nslookup", "nslookup_query"), ("dig", "dig_query"), ("host", "host_lookup")],
    )
    ...
    def test_dns_tools_comparison(
        self, mock_execute_command, sample_nslookup_output, sample_dig_output
    ):
